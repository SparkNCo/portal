...
**Preview Links banner:** above all of that, `PreviewLinksBanner` (`components/client/preview-links-banner.tsx`) shows the customer's Preview Links ("Test Environments" — e.g. a link to their staging URL) as `{text}: {url}`, clickable. Fetched by `slug` (`GET /users?type=customer-preview-links&slug={clientName}`), visible to every role. Settable from Admin → Users → a customer's **Profile** → Preview Links, or inline from the banner itself for admin/developer viewers (a small "Edit"/"Add" button next to the list — see `app/docs/ADMIN_FLOWS.md`). Renders nothing at all for a customer/stakeholder viewer when the customer has none set.
