...
Clicking an issue card opens the full **IssueDetailModal** — Description, Chat, Tests, Clarifications, Demo, and (for non-bug issues) Design tabs too, same as everywhere else in the app (see `app/docs/FEATURES_FLOWS.md`). A **pencil/edit button** on each card (new alongside the drill-down rework) opens `EditIssueModal` directly instead, and on save invalidates the `["roadmap", slug]` query so the timeline reflects the change.
